...
using BestGirl.Core.Discord.Rest.Repositories;
using BestGirl.Core.Discord.Rest.Repositories.Interfaces;
using System;

namespace BestGirl.Core.Discord.Rest
{
	public class RestClient : BaseRestRepository, IChannelRepository, IGatewayRepository, IUserRepository, IGuildRepository
	{
		...
		public async Task<Role> CreateGuildRoleAsync(ulong guildId)
		{
			return await PostJsonAsync<Role>($"guilds/{guildId}/roles", new { });
		}
	}
}
